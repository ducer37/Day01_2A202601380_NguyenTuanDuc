...
def call_openai(
    ...
    # Trả về nội dung (content) của phản hồi và độ trễ
...
def run_assistant(
    ...
    while True:
        ...
        turns += 1
        # Gom toàn bộ lịch sử thành 1 text để đếm token đầu vào chính xác
        prompt_text = "\n".join(m["content"] for m in messages)
        turn_tokens = count_tokens(prompt_text, OPENAI_MODEL) + count_tokens(reply, OPENAI_MODEL)
        tokens_used += turn_tokens
        turn_cost = estimate_cost(prompt_text, reply, OPENAI_MODEL)["total_cost"]
...
def batch_compare(prompts: list[str]) -> list[dict]:
    ...
    for prompt in prompts:
        ...
        # Sử dụng unpacking để tạo dict mới, tránh sửa trực tiếp kết quả của compare_models
        results.append({**res, "prompt": prompt})
    return results
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        p = shorten(r['prompt'])
        g4 = shorten(r['gpt4o_answer'])
        mn = shorten(r['mini_answer'])
        g4t = f"{r['gpt4o_time']:.2f}s"
        mnt = f"{r['mini_time']:.2f}s"
        table += f"{p:<40} | {g4:<40} | {mn:<40} | {g4t:<15} | {mnt:<15}\n"
    return table
