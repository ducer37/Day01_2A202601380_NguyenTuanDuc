...
def run_assistant(
    ...
    while True:
        ...
        turns += 1
        turn_tokens = count_tokens(user_msg, OPENAI_MODEL) + count_tokens(reply, OPENAI_MODEL)
        tokens_used += turn_tokens
        turn_cost = estimate_cost(user_msg, reply, OPENAI_MODEL)["total_cost"]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    header = f"{'Prompt':<40} | {'GPT-4o Response':<40} | {'Mini Response':<40} | {'GPT-4o Time':<12} | {'Mini Time':<12}\n"
    divider = "-" * len(header) + "\n"
    ...
    for r in results:
        ...
        table += f"{p:<40} | {g4:<40} | {mn:<40} | {g4t:<12} | {mnt:<12}\n"
    return table
